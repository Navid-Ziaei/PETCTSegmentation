...
echo "Copying results to local system"
mkdir -p $SCRIPTPATH/results/copied_output_$VOLUME_SUFFIX
docker run --rm -v autopet_baseline-output-$VOLUME_SUFFIX:/output \
            -v $SCRIPTPATH/results/copied_output_$VOLUME_SUFFIX:/local_output alpine cp -r /output/. /local_output/
